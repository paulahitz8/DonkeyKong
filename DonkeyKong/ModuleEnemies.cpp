...
void ModuleEnemies::HandleEnemiesDespawn()
{
	...
	for (uint i = 0; i < MAX_CAKES; ++i)
	{
		...
		{
			// Delete the enemy when it has reached the end of the screen
			if (enemiescakes[i]->position.x * SCREEN_SIZE < (App->render->camera.x) - SPAWN_MARGIN || (enemiescakes[i]->position.x == 120 && enemiescakes[i]->position.y == 120) || enemiescakes[i]->position.x == 255 || enemiescakes[i]->position.x == 0)
			{
